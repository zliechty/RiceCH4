...
WT.RS <- readRDS("WT.results.lfs.RS.RDS") %>% filter(OTU_ID %in% LRT.RS$OTU_ID) %>% select(Timepoint,Compartment,OTU_ID,LogFC=estimate)
WT.RP <- readRDS("WT.results.lfs.RP.RDS") %>% filter(OTU_ID %in% LRT.RP$OTU_ID) %>% select(Timepoint,Compartment,OTU_ID,LogFC=estimate)
WT.ES <- readRDS("WT.results.lfs.ES.RDS") %>% filter(OTU_ID %in% LRT.ES$OTU_ID) %>% select(Timepoint,Compartment,OTU_ID,LogFC=estimate)
```
