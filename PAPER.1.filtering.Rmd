...
```{r}
setwd("G:/My Drive/Sundar Lab/16S work/6.ARK_RP/ARK_ALL/or_uc/")
counts <- read.table("otu_table.txt", header = T, row.names = 1, sep = "\t")

setwd("G:/My Drive/Sundar Lab/16S work/6.ARK_RP/ARK_ALL/")
map <- read.table("ark.all.map.txt", header = T, sep = "\t") %>% 
...
These are likely contamination due to
```{r}
library(reshape2)
