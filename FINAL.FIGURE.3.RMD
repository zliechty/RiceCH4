...
RA_clean <- readRDS("RA_clean.pruned.n3.RDS")
  #rare <- t(vegan::rrarefy(t(counts_clean), sample = min(colSums(counts_clean))))
  #rare.clean <- rare[apply(rare, 1, occurence) >= ncol(rare) * 0.05,]
rare.clean <- read.table("G:/My Drive/for desktop/NEW.pruned.samples.noT3/rare.clean.txt")
...
g2r <- read.table("G:/My Drive/for desktop/NEW.pruned.samples.noT3/FAPROTAX/g2r.table2",sep="\t",header=T) %>% rename(OTU_ID = record) %>%
...
#ENRICHED FAMILIES
```{r}
fam.enrich <- readRDS("G:/My Drive/for desktop/NEW.pruned.samples.noT3/fam.enrich.RDS") %>%
...
setwd("G:/My Drive/for desktop/NEW.pruned.samples.noT3/DESEQ/LRT/")
LRT.RS <- readRDS("LRT.results.RS.RDS") %>% mutate(Compartment = "Rhizosphere")
LRT.RP <- readRDS("LRT.results.RP.RDS") %>% mutate(Compartment = "Rhizoplane")
LRT.ES <- readRDS("LRT.results.RS.RDS") %>% mutate(Compartment = "Rhizosphere")

LRT <- rbind(
  readRDS("LRT.results.ES.RDS") %>% mutate(Compartment = "Endosphere"),
  readRDS("LRT.results.RP.RDS") %>% mutate(Compartment = "Rhizoplane"),
  readRDS("LRT.results.RS.RDS") %>% mutate(Compartment = "Rhizosphere")) %>% 
